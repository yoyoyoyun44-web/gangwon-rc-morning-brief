...
REGIONS = {"춘천": (73, 134), "원주": (76, 122), "인제": (81, 138), "양구": (78, 139), "영월": (86, 119), "태백": (95, 119), "여주": (71, 121), "가평": (69, 133), "화천": (72, 139)}
...
def fetch_region(name, nx, ny, now, key):
    result = {"name": name, "nx": nx, "ny": ny, "temp": None, "sky": "API 키 미설정", "hourly": [], "error": None}
    if not key:
        result["error"] = "KMA_API_KEY missing"
        return result
    ...
    try:
        ...
        grouped = {}
        for item in items:
            date, tm = item.get("fcstDate"), item.get("fcstTime")
            if date and tm:
                grouped.setdefault((date, tm), {})[item.get("category")] = item.get("fcstValue")
        today = now.strftime("%Y%m%d")
        for (date, tm), values in sorted(grouped.items()):
            if date != today or int(tm[:2]) < now.hour:
                continue
            result["hourly"].append({"time": f"{tm[:2]}:{tm[2:]}", "temp": values.get("TMP"), "sky": sky_text(values.get("SKY"), values.get("PTY")), "pty": values.get("PTY", "0"), "pop": values.get("POP")})
            if len(result["hourly"]) >= 12:
                break
        if result["hourly"]:
            result["temp"] = result["hourly"][0]["temp"]
            result["sky"] = result["hourly"][0]["sky"]
        else:
            result["sky"] = "예보 없음"
...
def build_html(data, now_text):
    raw = json.dumps(data, ensure_ascii=False).replace("</", "<\\/")
    buttons = "".join(f'<button class="weather-pin" data-region="{name}" style="--x:{nx};--y:{ny}"><b>{name}</b><span class="weather-temp">-℃</span><small>불러오는 중</small></button>' for name, (nx, ny) in REGIONS.items())
    section = f'''<section class="page weather-page" id="weather-page"><div class="topline"><span>WEATHER / TODAY</span><span class="stamp">KMA FORECAST</span></div><div class="weather-kicker">강원·경기 주요 지역</div><h1>오늘의 날씨</h1><p class="weather-sub">지도에서 지역을 선택하면 오늘 시간대별 예보를 확인할 수 있습니다.</p><div class="weather-map"><div class="weather-map-shape"><div class="map-label">강원·경기<br><small>주요 지역 날씨</small></div></div>{buttons}</div><div class="weather-detail" id="weather-detail">지역을 선택해 주세요.</div><div class="weather-note">기상청 단기예보 기준 · 업데이트 {now_text}</div><script>window.MORNING_WEATHER={raw};</script><script>(function(){{const d=window.MORNING_WEATHER||{{}};const box=document.getElementById('weather-detail');function render(r){{const rows=(r.hourly||[]).map(x=>`<div class="hour-row"><b>${{x.time}}</b><span>${{x.sky}}</span><strong>${{x.temp??'-'}}℃</strong><small>강수 ${{x.pop??'-'}}%</small></div>`).join('');box.innerHTML=`<div class="detail-title"><b>${{r.name}}</b><strong>${{r.temp??'-'}}℃</strong><span>${{r.sky}}</span></div><div class="hourly-list">${{rows||'<div>시간대별 예보가 없습니다.</div>'}}</div>`;}}d.regions=d.regions||[];document.querySelectorAll('#weather-page .weather-pin').forEach(b=>{{const r=d.regions.find(x=>x.name===b.dataset.region);if(!r)return;b.querySelector('.weather-temp').textContent=(r.temp??'-')+'℃';b.querySelector('small').textContent=r.sky||'예보 확인 중';b.addEventListener('click',()=>render(r));}});}})();</script></section>'''
...
def main():
    ...
    regions = [fetch_region(name, nx, ny, now, service_key()) for name, (nx, ny) in REGIONS.items()]
