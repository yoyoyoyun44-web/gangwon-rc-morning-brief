...
LIFE_ASSET = ["연금보험", "연금저축", "연금상품", "연금 자산", "노후자금", "노후자금", "목돈마련", "목돈 마련", "자산관리", "자산 관리", "저축보험", "저축성보험", "저축성 보험", "적립보험", "적금", "저축", "연금 준비", "노후 준비", "노후자산", "노후 자산", "은퇴자금", "은퇴 자금"]
...
def collection_hours(now):
    kr = holidays.KR(years=[now.year, now.year - 1])
    previous_day = now.date() - timedelta(days=1)
    if now.weekday() == 0 or previous_day in kr:
        return 72
    return 24
...
def extract_items(source, html, cutoff):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not likely_title(title) or ("보도" not in title and not any(k in title for k in keywords)):
            continue
        href = urljoin(source["url"], a.get("href"))
        if not same_domain(href, source["url"]) or href.startswith(("javascript:", "mailto:")):
            continue
...
def dedup(existing, additions):
    ...
    for item in additions:
        ...
        if not url or url in seen_urls or not title:
            continue
        if any(title == normalize_title(old.get("title", "")) or SequenceMatcher(None, title, normalize_title(old.get("title", ""))).ratio() >= 0.84 for old in all_items):
            continue
...
def main():
    ...
    hours = collection_hours(now)
    cutoff = now - timedelta(hours=hours)
    print(f"공식기관 수집 기준: 최근 {hours}시간 / cutoff={cutoff.isoformat()}")
    try:
